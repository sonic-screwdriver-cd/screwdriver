...
defineSupportCode(({ Given, When, Then }) => {
    Given(/^a (valid|invalid)\b job-level template$/, function step(
        templateType
    ) {
        let targetFile = "";
        ...
        switch (templateType) {
            ...
            default:
                return Promise.reject(
                    "Template type is neither valid or invalid"
                );
        }
    ...
    Then(/^they are notified it has (no|some) errors$/, function step(
        quantity
    ) {
        switch (quantity) {
            case "no":
