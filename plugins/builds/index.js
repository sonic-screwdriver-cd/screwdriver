 ...
 */
async function getParentBuildStatus({ newBuild, joinListNames, pipelineId, buildFactory, triggerName }) {
    const upstream = newBuild.parentBuilds || {};
    let done = true;
    let hasFailure = false;
    const promisesToAwait = [];

    if (triggerName && !joinListNames.includes(triggerName)) {
        return {
            hasFailure: false,
            done: true
        };
...
const buildsPlugin = {
    ...
    async register(server, options) {
        ...
        server.expose('triggerNextJobs', async (config, app) => {
            ...
            const getExternalBuildsToRestart = async ({ externalEvent, nextJobNames, nextJobs }) => {
                // Remote join case
            ...
            const triggerJobsInExternalPipeline = async (externalPipelineId, joinObj) => {
                ...
                const externalBuildsToRestart = externalEvent
                    ? await getExternalBuildsToRestart({
                          externalEvent,
                          nextJobNames,
                          nextJobs
                      })
                ...
                if (!externalEvent || externalBuildsToRestart.length > 0) {
                    const { parentBuilds } = externalBuildsToRestart[0] || parseJobInfo({ current });
                ...
                for (const nextJobName of nextJobNames) {
                    ...
                    const nextBuild = externalBuildsToRestart.length
                        ? null
